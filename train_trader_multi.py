...
# Create the subfolder for saving results if it doesn't exist
if not os.path.exists(CONFIG["subfolder"]):
    os.makedirs(CONFIG["subfolder"])
...
def train(total_eps: int, market_params: tuple, test_freq: int, epsilon_start: float) -> DefaultDict:
    ...
    for episode in range(1, total_eps + 1):
        ...
        if episode % CONFIG["eps_per_evaluation"] == 0: 
            ...
            # Save the sellers q_table file for this policy evalutation (to keep track)
            new_file_name = os.path.join(CONFIG["subfolder"],f'q_table_seller_after_episode_{episode}.csv')
            shutil.copy('q_table_seller.csv', new_file_name)  
            
            # Save sa_counts to a CSV file with the episode number in the filename (to keep track)
            sa_counts_filename = os.path.join(CONFIG["subfolder"],f'sa_counts_episode_{episode}.csv')
